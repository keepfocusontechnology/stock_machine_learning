...
SVM寻找最小margin的两个平行超平面以区分样本类别。二维空间中，可由两条平行直线产生这个最小margin，这两个直线最少可由三个样本点唯一确定。这三个特征向量支撑了这两条直线，作为支持向量，故SVM 也称作**支持向量机.**
...
- Grid SearchCV 确定最佳参数

- 惩罚系数C：

  - 它在优化函数里主要是**`平衡支持向量的复杂度和误分类率这两者之间的关系，可以理解为正则化系数`**。
  - 当**惩罚系数C**比较大时，我们的**`损失函数也会越大`**，这意味着我们**`不愿意放弃比较远的离群点`**。这样我们会**`有更加多的支持向量`**，也就是说**`支持向量和超平面的模型也会变得越复杂，也容易过拟合`**。
  - 当**惩罚系数C**比较小时，意味我们**`不想理那些离群点`**，会选择**`较少的样本来做支持向量`**，最终的支持向量和超平面的模型也会简单。`scikit-learn`中默认值是1。

- gamma：

  - ` γ`主要定义了**`单个样本对整个分类超平面的影响`**。
  ...
  - 当`C`比较大、 `γ`比较大时，**`会有更多的支持向量，模型会比较复杂，较容易过拟合`**
  - 当`C`比较小、`γ`比较小时，**`模型会变得简单，支持向量的个数会少`**
...
- 交叉验证（Cross-validation）
  ...
  <img src="/Users/quat1ly/Library/Application Support/typora-user-images/image-20201130231353543.png" alt="image-20201130231353543" style="zoom:50%;" />

  从图中可以看出，随着模型的不断训练，训练集跟测试集的误差在不断缩小